...
class Plotter:
    ...
    def plot_2d(self, patterns, weights, threshold=None, title="Plot"):
        soa =np.array([[0,0,row[0], row[1]] for row in np.array(weights)])
        X,Y,U,V = zip(*soa)
        plt.figure()
        ax = plt.gca()
        plt.hold(True)
        ...
        for p in patterns:
                ax.scatter(p[0], p[1],c='r',marker='o')
        ...
        plt.show()
'''
p = [[[1],[0]],[[0.7071],[-0.7071]], [[0],[-1]]]
w = [[0,1],[0.7071,-0.7071],[-1,0]]
Plotter().plot_2d(p,w,title="Competitive neural network")
'''